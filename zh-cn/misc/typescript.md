...
在语言功能层面，`Props` 类型会被添加一层 `Readonly` 包装，因为组件 props 是不支持直接修改的：
...
在 Qingkuai 组件中，事件与其他非引用类型属性一样，都是通过内建的 `props` 对象访问的。也就是说，在组件标签上，属性名前的 `@` 和 `!` 前缀可以互换，`@` 主要用于语义标识，表示该属性是可调用的方法。当你为组件添加属性时，输入 `@` 会触发属性名自动补全，这些被提示的名称正是被推断为事件的属性。只要组件的 `Props` 类型中某个属性的值是函数类型，Qingkuai 语言服务器就会将其标记为事件候选项，并在你输入 `@` 时提供相应的属性名补全提示。下面代码中 `Props` 的两个属性都会被推断为事件：
