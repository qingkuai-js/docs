...
4. 用户比较在意运行时性能时，可在 `.qingkuairc` 中搭配 `"reactivityMode": "shallow"` 与 `"allowConstReactive": false` 进入信号模式（详见 [optimization.md](./misc/optimization.md)）；此模式下只能通过为标识符重新赋值来触发更新。
